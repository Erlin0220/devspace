...
Journal MCP tools are registered by the same local extension facade and are implemented in `src/journal-tools.ts` and `src/journal-mlflow.ts`. The Python runner lives at `scripts/mlflow-journal.py`.

Runtime journal databases such as `mlflow.db` are local state and should not be committed.
