...
function render(state) {
  ...
  $('installation-status').textContent = install ? `${({ queued: '安装任务已提交', staging: '正在准备独立安装目录，当前服务继续运行', switching: '正在切换并验证 Runtime', installed: '安装已完成', failed: `安装失败：${install.error ?? '查看诊断信息'}` })[install.status] ?? ''}${install.warning ? ` · ${install.warning}` : ''}` : '';
