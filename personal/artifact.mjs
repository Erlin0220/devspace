...
export async function recordInstallable(root, stages) {
  ...
  const commit = await revision(root); const payload = await payloadDigest(root);
...
export async function verifyInstallable(root) {
  ...
  if (receipt.schema !== 1 || receipt.platform !== process.platform || receipt.arch !== process.arch || receipt.node !== process.version || receipt.commit !== await revision(root)) throw new Error('Candidate revision/platform/Node version differs from its verification receipt');
